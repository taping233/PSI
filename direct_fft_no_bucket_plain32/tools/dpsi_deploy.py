#!/usr/bin/env python3
"""Deployable DPSI Method 2 prototype with dynamic party updates.
...
def default_poly_backend() -> str:
    ...
    if env:
        return env
    return str(Path(__file__).resolve().parents[1] / "build-rie" / "dpsi_poly_backend")
...
def load_state(path: Path, default_role: str) -> Dict[str, Any]:
    ...
    return {"schema": 1, "role": default_role, "field_prime": FIELD_PRIME, "parties": {}}
...
class JsonHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _compute_query_result(self, query: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        rounds = int(query.get("rounds", ["2"])[0])
        local = aggregate_parties(state.get("parties", {}), rounds)
        cloud_url = self.server.cloud_url.rstrip("/") + f"/aggregate?rounds={rounds}"
        ...
        if rounds < 2:
            raise RuntimeError("Method 2 query requires at least two rounds")
        reconstructed = []
...
def query_result(args: argparse.Namespace) -> None:
    url = args.query_url.rstrip("/") + f"/result?rounds={args.rounds}"
    print(json.dumps(http_json("GET", url, None, args.token, args.insecure_tls), sort_keys=True))


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Real-process DPSI Method 2 deployment and dynamic updates")
    ...
    upd.add_argument("--rounds", type=int, default=2)
    ...
    qr.add_argument("--rounds", type=int, default=2)
